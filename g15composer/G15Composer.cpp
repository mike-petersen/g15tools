...
G15Composer::run ()
{
  int retval = pthread_create (&this->thread, NULL, G15Composer::threadEntry,
			       (void *) this);
  return retval;
}

void
G15Composer::updateScreen (bool force)
{
...
void
G15Composer::handlePixelCommand (string const &input_line)
{
  switch (input_line[1])
    {
    case 'S':
	...
	g15r_pixelReverseFill (canvas, params[0], params[1], params[2],
			       params[3], fill, color);
	break;
      }
    case 'O':
	...
	if (exp != (int) input_line.length () - ofs)
	  {
	    cout << "Error in Pixel Overlay Command (" << params[2] << "x" <<
	      params[3] << ")" << endl;
	...
	g15r_pixelOverlay (canvas, params[0], params[1], params[2], params[3],
			   output_line);
	break;
      }
    case 'B':
    ...
    case 'C':
      {
	g15r_clearScreen (canvas, input_line.length () < 4
			  || input_line[3] == '1');
	break;
      }
    default:
...
void
G15Composer::handleDrawCommand (string const &input_line)
{
  switch (input_line[1])
    {
    case 'L':
	...
	g15r_drawLine (canvas, params[0], params[1], params[2], params[3],
		       params[4]);
	break;
      }
    case 'C':
	...
	g15r_drawCircle (canvas, params[0], params[1], params[2], params[3],
			 params[4]);
	break;
      }
    case 'R':
	...
	g15r_drawRoundBox (canvas, params[0], params[1], params[2], params[3],
			   params[4], params[5]);
	break;
      }
    case 'B':
	...
	g15r_drawBar (canvas, params[0], params[1], params[2], params[3],
...
void
G15Composer::handleModeCommand (string const &input_line)
{
...
#ifdef TTF_SUPPORT
void
G15Composer::handleFontCommand (string const &input_line)
{
  string parse_line;
  ...
  switch (input_line[1])
	...
	for (unsigned int row = 0; row < lines.size (); ++row)
	  {
	    g15r_ttfPrint (canvas, x, y, fontsize, fontface, color, center,
			   (char *) lines[row].c_str ());
	  }
	updateScreen (false);
	break;
      }
...
void
G15Composer::handleTextCommand (string const &input_line)
{
  string parse_line;
  int params[4] = { 0, 0, 0, 0 };
  int size = -1;
  ...
  for (row = 0; row < lines.size (); ++row)
      ...
      if (center)
	{
	  ...
	  if (dispcol < 0)
	    dispcol = 0;
	  g15r_renderString (canvas, stringOut, row, size, dispcol,
			     params[1]);
	}
      else
	g15r_renderString (canvas, stringOut, row, size, params[0],
			   params[1]);
    }
...
void
G15Composer::handleKeyCommand (string const &input_line)
{
  int params[2] = { 0, 0 };
  get_params (params, input_line, 3, 2);
...
void
G15Composer::handleLCDCommand (string const &input_line)
{
  int params[1] = { 0 };
...
void
G15Composer::parseCommandLine (string cmdline)
{
  int i = 0;
...
void
G15Composer::fifoProcessingWorkflow ()
{
  string line = "";
  int fd = doOpen ();
...
/* static */
void *
G15Composer::threadEntry (void *pthis)
{
