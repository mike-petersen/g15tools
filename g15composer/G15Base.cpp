...
G15Base::G15Base (string filename)
{
  ...
  if (mkfifo (fifo_filename.c_str (), mode))
    {
      cout << "Error, could not create FIFO " << fifo_filename << " aborting"
	<< endl;
      leaving = true;
      keepFifo = true;
    }
...
void
G15Base::handleScreenCommand (std::string const &input_line)
{
  switch (input_line[1])
    {
    case 'N':
	...
	for (i = 0; i < (int) parse_line.length (); ++i)
	  if (parse_line[i] == '\"')
	      ...
	      else if (in_line)
		{
		  in_line = false;
		  string temp =
		    parse_line.substr (line_start, i - line_start);
		  newpipe = temp.c_str ();
		}
	      else
		{
...
G15Base::doOpen ()
{
  ...
  if (fifo_filename == "-")
  ...
  else
    {
      ...
      if (fd == -1)
	{
	  cout << "Error, could not open " << fifo_filename << " aborting" <<
	    endl;
	  return -1;
	}
    }
...
int
G15Base::get_params (int *params, string const &input_line, int start,
		     int count)
{
  char tmp[10];
  int ti = 0;
